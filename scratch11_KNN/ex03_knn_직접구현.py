...
def train_test_split(X, y, test_size):
    ...
    X_train = X[indices[:cut]] # X의 indices를 cut까지만큼 열을 갖는 리스트로 X_train 생성(cut=4면 처음부터 4열까지)
    X_test = X[indices[cut:]] # X의 indices를 cut부터 끝까지만큼 열을 갖는 리스트로 X_test 생성(cut=4면 4열부터 끝까지)
